...
export const incomeCsvToJson = (csv: string): RawIncomeData[] => {
    ...
    for (let i = headerRowIndex + 1; i < lines.length; i++) {
        ...
        // Extract income values for each year
        for (const { index, year } of yearColumns) {
            if (index < values.length) {
                ...
                if (valueStr && valueStr !== '' && !isNaN(parseFloat(valueStr))) {
                    const incomeValue = parseFloat(valueStr);
                    if (!isNaN(incomeValue) && isFinite(incomeValue)) {
                        incomeData.push({
                            countryCode,
